...
@app.cell
def _(pd):
    data = pd.read_csv('https://raw.githubusercontent.com/TimGyger/VIF/refs/heads/main/Real%20World/Gaussian/Elevators/elevators.txt', sep=" ", header=None)
    return (data,)
        ...
        def __init__(self, train_x, train_y, likelihood):
            ...
            self.covar_module = ScaleKernel(
                gpytorch.kernels.GridInterpolationKernel(MaternKernel(batch_shape=torch.Size([train_x.size(-1)]),nu=3/2), 
                                        grid_size=round(1000/d), num_dims=1)
            )
...
def _(
    ...
    class SGPR(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood):
            ...
            self.covar_module = InducingPointKernel(self.base_covar_module, inducing_points=train_x[:num_inducing_points, :].clone(), likelihood=likelihood)
...
def _():
    ...
    num_inducing_points_2 = 200
    num_Vecchia_neighbors = 30
    return likelihood_1, num_Vecchia_neighbors, num_inducing_points_2
...
def _(
    ...
    for j_1 in range(5):
        ...
        print(X_train_1.shape)
        print(y_train_1.shape)
        model_vif_corr = gpb.GPModel(gp_coords=X_train_1, cov_function='matern_ard', cov_fct_shape=1.5, likelihood=likelihood_1, vecchia_ordering='random', num_neighbors=num_Vecchia_neighbors, num_ind_points=num_inducing_points_2, ind_points_selection='kmeans++', matrix_inversion_method='cholesky', gp_approx='full_scale_vecchia_correlation_based')
        model_vif_eucl = gpb.GPModel(gp_coords=X_train_1, cov_function='matern_ard', cov_fct_shape=1.5, likelihood=likelihood_1, vecchia_ordering='random', num_neighbors=num_Vecchia_neighbors, num_ind_points=num_inducing_points_2, ind_points_selection='kmeans++', matrix_inversion_method='cholesky', gp_approx='full_scale_vecchia')
...
def _(mo):
    mo.md(
        r"""
        ### VIF Approximation (estimate shape)
        """
    )
    return
...
def _():
    ...
    return likelihood_2, num_Vecchia_neighbors_1, num_inducing_points_3
...
def _(
    ...
    for j_2 in range(5):
        ...
        print(X_train_2.shape)
        print(y_train_2.shape)
        model_vif_corr_1 = gpb.GPModel(gp_coords=X_train_2, cov_function='matern_ard_estimate_shape', cov_fct_shape=15, likelihood=likelihood_2, vecchia_ordering='random', num_neighbors=num_Vecchia_neighbors_1, num_ind_points=num_inducing_points_3, ind_points_selection='kmeans++', matrix_inversion_method='cholesky', gp_approx='full_scale_vecchia_correlation_based')
...
def _():
    ...
    return likelihood_3, num_Vecchia_neighbors_2, num_inducing_points_4
...
def _():
    ...
    return likelihood_4, num_Vecchia_neighbors_3, num_inducing_points_5
...
def _(
    ...
    for j_4 in range(5):
        ...
        metric = 'mse'
        model_vif_corr_3 = gpb.GPModel(gp_coords=X_train_4, cov_function='matern_ard', cov_fct_shape=1.5, likelihood=likelihood_4, vecchia_ordering='random', num_neighbors=num_Vecchia_neighbors_3, num_ind_points=num_inducing_points_5, ind_points_selection='kmeans++', matrix_inversion_method='cholesky', gp_approx='full_scale_vecchia_correlation_based')
